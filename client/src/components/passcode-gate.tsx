...
export function PasscodeGate({ children }: { children: React.ReactNode }) {
  ...
  // Returning user: already has access grant, waiting for auth check to complete
  ...
  function handleLogin(provider: "replit" | "google") {
    ...
    const redirectParam = isMobile ? "mobile" : "web";
    const base = provider === "google" ? "/api/login/google" : "/api/login";
    const loginUrl = `${base}?redirect=${redirectParam}`;
  ...
  return (
    <StartupShell fading={!visible}>
      <PanelCard key="login">
        ...
        <p className="text-slate-500 text-xs text-center mt-4">
          Sign in with your Replit or Google account to continue.
        </p>
      </PanelCard>
    </StartupShell>
  );
