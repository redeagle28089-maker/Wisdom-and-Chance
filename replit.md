...
- **Admin AI Generator (v2.7.0):** Admin-only batch generator at `/admin/ai-generator` that uses Gemini 2.5 Flash to produce 1–10 fully playable units or commanders at a time, biased by element / power range / cost range / free-text style. The single source of truth for what counts as a playable commander effect lives in `shared/models/cards.ts` (`ALLOWED_ABILITY_EFFECTS` — currently 23 effects matching the switch in `gameEngine.processAbility`); both the tightened `commanderAbilityEffectSchema` (with `superRefine` enforcing per-effect `value`/`target` shape) and the AI prompt read from this list, so the generator can never invent an effect that the engine cannot run. Endpoints: `POST /api/admin/generate-cards` (returns `{ candidates, rejectedCount, rejectedDetails, totalReturnedByAi }` — every candidate parses through `insertCardSchema` / `insertCommanderSchema` server-side), `POST /api/admin/generated-cards/save` (re-validates and inserts via `storage.createCard` / `storage.createCommander`), `GET /api/admin/allowed-effects` (returns the full cheat-sheet). The page renders previews via the existing `CardWithPopup` / `CommanderWithPopup` components and exposes a collapsible cheat-sheet of all 23 implemented effects so the admin can read what each one does before approving. Smoke test: `node scripts/test-ai-generator.mjs` against a running dev server.
...
- **Lag handling (v2.6.3):** Resilience + UX feedback for slow / unreliable connections without changing game rules. **Server (`server/websocket.ts` + `server/gameEngine.ts`):** Per-WS pong watchdog pings every 15 s and `terminate()`s any socket whose pong is older than `MP_HEARTBEAT_TIMEOUT_MS` (default 45 s) — so dead NAT/proxy connections trip the existing disconnect-forfeit timer instead of becoming "ghost players". Per-game turn timer (`MP_TURN_TIMEOUT_MS`, default 60 s) auto-passes the active player's required action on inactivity (`autoActionForPlayer` covers draw / deploy-as-pass / end_turn); **3 consecutive timeouts → auto-forfeit** with `game_over_from_timeout` broadcast. Every `game_state` payload is stamped with a monotonic `seq` + `turnDeadline` + `responsiblePlayerIds` + `consecutiveTimeouts` so clients can render countdowns and dedupe stale snapshots. Incoming `game_action` accepts an optional `clientActionId`; the engine's `markActionSeen(gameId, playerId, actionId)` ring-buffer ignores duplicates from the same player and re-acks via `action_ack { duplicate: true }`. **Client (`client/src/hooks/use-websocket.ts`):** Heartbeat sends `{type:"ping"}` every 15 s, counts missed pongs, and force-closes after 3 missed (any inbound traffic resets the counter). Outbound queue buffers messages while `CONNECTING/CLOSED`, flushes them on `open`, drops entries older than 30 s, and exposes `pendingCount`. `sendGameAction` now generates and returns a `clientActionId` for tracking. **Web UI (`client/src/pages/game-board.tsx`):** Multiplayer-only "Sending…" / "Reconnecting… (N queued)" / "Your turn — Xs left" / "Opponent's turn — thinking… Xs" / turn-timeout toasts. Practice/AI mode: `isAIProcessing` input lock around `executeAITurn`, ~250 ms-per-card deploy stagger, 700 ms cadence between combat-resolution phases, "AI is thinking…" badge, and disabled draw/deploy/combat buttons while the AI acts. **Mobile (`mobile/app/game/board.tsx`):** "AI is thinking…" badge added (input lock + 700 ms combat stagger were already present). **Admin/test endpoints (`server/routes.ts`):** `POST /api/admin/test/turn-timeout` and `POST /api/admin/test/heartbeat-timeout` (admin-only, 100–600000 ms) so the hardening test can exercise these paths in seconds. **Coverage:** new checkpoints in `scripts/test-multiplayer-hardening.mjs` for heartbeat-loss termination, turn-timer auto-pass, 3-strike auto-forfeit, and `clientActionId` idempotency.
- **Web build freshness + service worker (v2.6.4):** `client/public/sw.js` now uses **network-first for HTML** (anything matching `request.mode === 'navigate'`, `Accept: text/html`, or extension-less paths like `/`) so a republish takes effect on the next page load instead of being shadowed by the cached `index.html`. Static assets (`/icon-*.png`, `/favicon.png`, `/manifest.json`) remain cache-first. `CACHE_NAME` bumped from `wc-tcg-v1` → `wc-tcg-v2` so existing service workers in returning users' browsers self-evict on `activate`. The dead-code `redirectToLogin` helper was removed from `client/src/lib/auth-utils.ts` (it had zero call sites and could mislead future contributors); `isUnauthorizedError` is retained. Local `dist/` was rebuilt via `npm run build` so the artifact is current; `suggest_deploy` was triggered so the user can publish the refreshed build to `.replit.app`. Once published, the current `PasscodeGate` landing page (logo + "Sign in with Replit" button — no auto-redirect to OIDC) will be what unauthenticated visitors see.
- **Dev OAuth callback routing (v2.6.2):** `.replit` port mappings flipped so the **web app** (`localPort=5000`) is exposed on the canonical bare-domain (`externalPort=80`), and the **mobile Expo dev server** (`localPort=8080`) moved to `externalPort=8080`. Previously mobile Expo hijacked the bare-domain (`externalPort=80`), causing the post-OAuth redirect to `https://<repl>.replit.dev/api/callback` to land on the mobile router and show "This screen doesn't exist". Mobile dev is reachable via Expo's dedicated `REPLIT_EXPO_DEV_DOMAIN` subdomain or `:8080`. Production deployment is unaffected (its `[deployment]` block runs only the bundled web app on port 8080→80).
