  ...
  economy: {
    ...
    endpoints: {
      ...
      shopCatalog: {
        ...
        requiresAuth: false,
        description: "Get all available pack types in the shop catalog.",
        response: {
          type: "array",
          items: {
            id: { type: "string", description: "Pack type ID (e.g. standard, premium, fire)" },
            name: { type: "string" },
            description: { type: "string" },
            costGold: { type: "number" },
            costGems: { type: "number" },
            cardsPerPack: { type: "number" },
            elementFilter: { type: "string|null", description: "Element filter (e.g. Fire, Water) or null" },
            guaranteedMinRarity: { type: "string|null", description: "Minimum guaranteed rarity or null" },
          },
        },
      },
      shopDailyDeal: {
        ...
        response: {
          ...
          originalCostGold: { type: "number" },
          discountedCostGold: { type: "number" },
          discountPercent: { type: "number" },
          featuredCardId: { type: "string|null" },
      ...
      shopPurchase: {
        ...
        requiresAuth: true,
        description: "Purchase and open a specific pack type. Supports daily deal discounts. Returns cards with full details for the pack opening animation.",
        body: {
          packTypeId: { type: "string", required: true, description: "Pack type to purchase (standard, premium, fire, water, earth, air, nature)" },
          useDailyDeal: { type: "boolean", required: false, description: "Set true to apply daily deal discount if the pack matches today's deal" },
        },
        response: {
          packTypeId: { type: "string" },
          packName: { type: "string" },
          cards: { type: "array", description: "Array of { cardId, rarity, isNew, cardName, element, power }" },
          costGold: { type: "number" },
          remainingGold: { type: "number" },
          remainingGems: { type: "number" },
        },
        errors: {
          400: "Invalid pack type / Not enough gold",
        },
      },
    },
  },
