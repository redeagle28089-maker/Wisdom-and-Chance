...
import { Card, Commander, CommanderAbility } from '@/lib/api';
import { GameState, GamePhase, DeployedCard, RoundResult, AbilityEffect, PlayerState, GAME_CONSTANTS } from '@/lib/game-engine';
...
function isAbilityUsableInPhase(abilityPhase: string, currentPhase: GamePhase): boolean {
  const p = abilityPhase.toLowerCase();
  if (p === 'deployment' || p === 'deploy') return currentPhase === 'deployment' || currentPhase === 'card_draw';
  if (p === 'combat') return currentPhase === 'combat';
  if (p === 'draw') return currentPhase === 'card_draw';
  return false;
}
  ...
  return (
    <View style={styles.phaseRow}>
      {phases.map((p, i) => {
        ...
        return (
          <View key={i} style={[styles.phaseDot, active && { backgroundColor: p.color + '30', borderColor: p.color }]}>
            <Ionicons name={p.icon as IoniconsName} size={8} color={active ? p.color : '#475569'} />
          </View>
        );
      })}
    </View>
...
function HPBar({ hp, max, isPlayer, label }: { hp: number; max: number; isPlayer: boolean; label: string }) {
  ...
  const barColor = hp > max * 0.5 ? '#22C55E' : hp > max * 0.25 ? '#F59E0B' : '#EF4444';
  const borderCol = isPlayer ? '#22C55E30' : '#EF444430';
  return (
    <View style={[styles.hpBarWrap, { borderColor: borderCol }]}>
...
function BattlefieldCard({ card, faceDown, isPlayer, canRemove, onPress, cardWidth }: {
  card: DeployedCard; faceDown: boolean; isPlayer: boolean; canRemove?: boolean; onPress?: () => void; cardWidth: number;
}) {
  const elColor = getElementColor(card.element || 'fire');
      ...
      <View style={[styles.bfCard, styles.bfCardFaceDown, { width: cardWidth }]}>
        <LinearGradient colors={['#581C87', '#1E293B']} style={styles.bfCardInner}>
          <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
            <MaterialCommunityIcons name="cards" size={14} color="#7C3AED" />
          </View>
        </LinearGradient>
      </View>
    );
  ...
  return (
    <Pressable style={[styles.bfCard, { width: cardWidth, borderColor: elColor + '80' }]} onPress={onPress}>
      <View style={styles.bfCardInner}>
        {card.id ? (
          ...
          <LinearGradient colors={[getElementBg(card.element), '#0F172A']} style={StyleSheet.absoluteFillObject} />
        )}
        <View style={styles.bfPowerBadge}>
          <Text style={styles.bfPowerText}>{card.currentPower ?? card.power}</Text>
        </View>
        {card.trait && (
          <View style={[styles.bfTraitBadge, { backgroundColor: '#9333EACC' }]}>
            <Text style={styles.bfTraitText}>{card.traitValue ?? 0}</Text>
            <Text style={styles.bfTraitLabel}>{card.trait?.charAt(0)}</Text>
          </View>
        )}
        <View style={styles.bfCardNameBar}>
          <Text style={styles.bfCardName} numberOfLines={1}>{card.name}</Text>
        </View>
        {canRemove && (
...
function HandCardComponent({ card, onPress, onHoldPreview, selected, cardWidth }: {
  card: Card; onPress: () => void; onHoldPreview?: () => void; selected: boolean; cardWidth: number;
}) {
  const elColor = getElementColor(card.element || 'fire');
  const fallbackHandArt = card.imageUrl || ELEMENT_ART[(card.element || '').toLowerCase()];
  const nameSize = cardWidth < 40 ? 4 : 5;
  return (
    <Pressable
      style={[styles.handCard, { width: cardWidth, borderColor: elColor + '80' }, selected && styles.handCardSelected]}
      onPress={onPress}
    ...
    >
      ...
      <View style={styles.handPowerBadge}>
        <Text style={styles.handPowerText}>{card.power}</Text>
      </View>
      {card.trait && (
        <View style={[styles.handTraitBadge, { backgroundColor: '#9333EACC' }]}>
          <Text style={styles.handTraitVal}>{card.traitValue ?? 0}</Text>
        </View>
      )}
      <View style={[styles.handCardNameWrap, { height: nameSize + 6 }]}>
        <Text style={[styles.handCardNameText, { fontSize: nameSize }]} numberOfLines={1}>{card.name}</Text>
      </View>
    </Pressable>
  );
}

function adaptServerState(raw: any): PvPGameState {
  if (!raw) return raw;
  const player = raw.player || {};
  const opponent = raw.opponent || {};
  return {
    phase: raw.phase || 'deployment',
    round: raw.round || 1,
    player: {
      hp: player.hp ?? 40,
      hand: player.hand || [],
      deployed: (player.deployed || player.field || []).map((c: any) => ({
        ...c,
        currentPower: c.currentPower ?? c.power,
      })),
      deck: player.deck || { count: player.deckCount ?? 0 },
      commander: player.commander || { name: 'Commander', title: '', element: 'fire', abilities: [] },
      victoryCurrency: player.victoryCurrency ?? player.advanceCounters ?? 0,
      withdrawalCurrency: player.withdrawalCurrency ?? player.defeatCounters ?? 0,
      usedAbilities: player.usedAbilities || [],
    },
    opponent: {
      hp: opponent.hp ?? 40,
      handCount: opponent.handCount ?? opponent.hand?.length ?? 0,
      deployed: (opponent.deployed || opponent.field || []).map((c: any) => ({
        ...c,
        currentPower: c.currentPower ?? c.power,
      })),
      deckCount: opponent.deckCount ?? opponent.deck?.count ?? 0,
      commander: opponent.commander || { name: 'Opponent', title: '', element: 'fire', abilities: [] },
      victoryCurrency: opponent.victoryCurrency ?? opponent.advanceCounters ?? 0,
      withdrawalCurrency: opponent.withdrawalCurrency ?? opponent.defeatCounters ?? 0,
    },
    cardsDeployedPerTurn: raw.cardsDeployedPerTurn ?? raw.cardsToDeploy ?? 2,
    cardsDrawnPerTurn: raw.cardsDrawnPerTurn ?? raw.cardsToDraw ?? 2,
...
export default function PvPGameBoardScreen() {
  const insets = useSafeAreaInsets();
  const { width: screenW, height: screenH } = useWindowDimensions();
  const params = useLocalSearchParams<{ roomId: string; gameId: string }>();
  ...
  useEffect(() => {
    ...
    const matchesGame = (data: any) =>
      (gameId && data.gameId === gameId) || (data.roomId === roomId);

    const unsubs = [
      ws.subscribe('game_state', (data: any) => {
        if (!matchesGame(data)) return;
        const state = data.state || data.payload || data;
        if (state.player || state.phase) {
          setGameState(adaptServerState(state));
      ...
      ws.subscribe('game_action', (data: any) => {
        if (!matchesGame(data)) return;
        ...
        if (actionType === 'combat_result') {
          ...
          (async () => {
            ...
            const result = action.roundResult;
            if (result) {
              safeHaptic(() => Haptics.notificationAsync(
                result.winner === 'p1' ? Haptics.NotificationFeedbackType.Success :
                result.winner === 'p2' ? Haptics.NotificationFeedbackType.Error :
            ...
            if (action.updatedState) {
              setGameState(adaptServerState(action.updatedState));
              if (action.updatedState.phase === 'game_over') {
                setShowGameOver(true);
              }
            }
          })();
        } else if (actionType === 'opponent_deployed' || actionType === 'deploy_card') {
          safeHaptic(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light));
          if (action.state) {
            setGameState(adaptServerState(action.state));
          }
        } else if (actionType === 'game_over') {
          if (action.finalState) setGameState(adaptServerState(action.finalState));
          if (action.state) setGameState(adaptServerState(action.state));
          setShowGameOver(true);
        } else if (actionType === 'turn_ended' || actionType === 'end_turn') {
          setWaitingForOpponent(false);
          if (action.state) {
            setGameState(adaptServerState(action.state));
          }
        } else if (actionType === 'rematch_requested') {
          setOpponentRematch(true);
        } else if (actionType === 'rematch_accepted') {
        ...
        } else if (actionType === 'phase_change') {
          setWaitingForOpponent(false);
          if (action.state) {
            setGameState(adaptServerState(action.state));
          }
      ...
      ws.subscribe('game_over', (data: any) => {
        if (!matchesGame(data)) return;
        if (data.finalState) setGameState(adaptServerState(data.finalState));
        setShowGameOver(true);
      }),
      ws.subscribe('opponent_disconnected', (data: any) => {
        if (!matchesGame(data)) return;
        setOpponentDisconnected(true);
        setDisconnectTimer(data.timeout || 60);
      }),
      ws.subscribe('opponent_reconnected', (data: any) => {
        if (!matchesGame(data)) return;
  ...
  if (!gameState) {
    const loadingContent = (
      <LinearGradient colors={['#0F172A', '#1E1033', '#0F172A']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={[styles.loadingText, { fontSize: 10, color: '#64748B', marginTop: 4 }]}>
  ...
  const isDeployPhase = gameState.phase === 'deployment' || gameState.phase === 'card_draw';
  const isGameOver = gameState.phase === 'game_over';
  const isRoundEnd = gameState.phase === 'round_end';

  const cmdElColor1 = getElementColor(p.commander.element);
  const cmdElColor2 = getElementColor(opp.commander.element);
    ...
    >
      ...
      <View style={[styles.header, { height: headerH }]}>
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.headerPlayerName} numberOfLines={1}>{p.commander.name}</Text>
            <Text style={styles.headerPlayerTitle} numberOfLines={1}>{p.commander.title}</Text>
          </View>
        </View>

        <View style={styles.headerCenter}>
          <View style={styles.roundPhaseRow}>
            <View style={[styles.roundBadge, { backgroundColor: '#EF444425' }]}>
              <Text style={[styles.roundText, { color: '#EF4444' }]}>PvP</Text>
            </View>
            <View style={styles.roundBadge}>
              <Text style={styles.roundText}>R{gameState.round}</Text>
            </View>
            <PhaseIndicator phase={gameState.phase} />
            <View style={[styles.phaseLabelBadge, { backgroundColor: getPhaseColor(gameState.phase) + 'CC' }]}>
              <Text style={styles.phaseLabelText}>{getPhaseLabel(gameState.phase)}</Text>
            </View>
          </View>
          <View style={styles.hpSection}>
            <HPBar hp={opp.hp} max={GAME_CONSTANTS.initialHP} isPlayer={false} label="Opp" />
            <HPBar hp={p.hp} max={GAME_CONSTANTS.initialHP} isPlayer={true} label="You" />
          </View>
        </View>
        ...
        <View style={styles.headerRight}>
          <View style={styles.currencyCol}>
            <View style={styles.currencyBadge}>
              ...
              <Text style={styles.currencyText}>{p.victoryCurrency}</Text>
            </View>
            <View style={styles.currencyBadge}>
              <Ionicons name="shield" size={9} color="#94A3B8" />
              <Text style={styles.currencyText}>{p.withdrawalCurrency}</Text>
            </View>
          </View>
          ...
          <Pressable style={styles.cmdAvatar} onPress={() => setShowDialog('oppCommanderInfo')}>
            <LinearGradient colors={[getElementColor(opp.commander.element), '#334155']} style={styles.cmdAvatarInner}>
              <MaterialCommunityIcons name="shield-crown" size={14} color="#fff" />
            </LinearGradient>
          </Pressable>
        </View>
      ...
      <View style={[styles.oppHandRow, { height: oppHandRowH }]}>
        <View style={styles.oppHandInfo}>
          ...
          <Text style={styles.oppHandLabel} numberOfLines={1}>{opp.commander.name}</Text>
          <View style={styles.zoneCurrRow}>
            <View style={styles.zoneCurrBadge}>
              <Ionicons name="star" size={6} color="#F59E0B" />
              <Text style={styles.zoneCurrText}>{opp.victoryCurrency}</Text>
            </View>
            <View style={styles.zoneCurrBadge}>
              <Ionicons name="shield" size={6} color="#94A3B8" />
              <Text style={styles.zoneCurrText}>{opp.withdrawalCurrency}</Text>
            </View>
          </View>
          <Text style={styles.oppHandCount}>D:{opp.deckCount} H:{opp.handCount}</Text>
        </View>
      ...
      <View style={[styles.fieldZone, styles.opponentZone, { height: fieldZoneH }]}>
        <View style={styles.zoneHeader}>
          ...
          <Text style={styles.zoneCountText}>{opp.deployed.length} deployed</Text>
        </View>
        <View style={styles.zoneCards}>
          {opp.deployed.length === 0 ? (
      ...
      <View style={[styles.centerStrip, { height: centerStripH }]}>
        {combatPhase >= 0 && combatSteps.length > 0 ? (
          ...
          <View style={styles.vsDivider}>
            ...
            <View style={[styles.vsBadge, { borderColor: '#EF444440' }]}>
              <MaterialCommunityIcons name="sword-cross" size={10} color="#EF4444" />
            </View>
            <View style={styles.vsLine} />
          </View>
        )}
          ...
          <View style={styles.zonePlayerInfo}>
            ...
            {isDeployPhase && (
              <View style={styles.deployCounter}>
                <Text style={styles.deployCounterText}>{p.deployed.length}/{gameState.cardsDeployedPerTurn}</Text>
              </View>
            )}
          </View>
          <Text style={styles.zoneCountText}>Deck: {p.deck.count}</Text>
        </View>
        <View style={styles.zoneCards}>
          {p.deployed.length === 0 ? (
        ...
        <View style={styles.bottomTopRow}>
          <View style={styles.handToggle}>
            <Pressable style={[styles.handToggleBtn, handView === 'units' && styles.handToggleBtnActiveUnits]} onPress={() => setHandView('units')}>
              ...
              <Text style={[styles.handToggleText, handView === 'units' && { color: '#BBF7D0' }]}>Hand ({p.hand.length})</Text>
            </Pressable>
          ...
          {isDeployPhase && !waitingForOpponent && (
            <Pressable
            ...
            >
              ...
              <Text style={styles.battleButtonText}>End Turn</Text>
            </Pressable>
          )}
        </View>
            ...
            <View style={styles.handRow}>
              {p.hand.map((c, i) => (
                <HandCardComponent
                  key={c.id + '-h-' + i}
                  card={c}
                  onPress={() => isDeployPhase && !waitingForOpponent ? handleDeploy(i) : setSelectedHandIndex(selectedHandIndex === i ? null : i)}
              ...
              {p.hand.length === 0 && <Text style={styles.zoneEmptyText}>No cards in hand</Text>}
            </View>
          ) : (
            <View style={styles.handRow}>
              {p.commander.abilities.map((ab) => {
                const used = p.usedAbilities.includes(ab.id);
                const canAfford = (ab.victoryCost === 0 || p.victoryCurrency >= ab.victoryCost) && (ab.withdrawalCost === 0 || p.withdrawalCurrency >= ab.withdrawalCost);
                const phaseMatch = isAbilityUsableInPhase(ab.phase, gameState.phase);
      ...
      {/* === GAME OVER OVERLAY (inline, not Modal) === */}
      {showGameOver && isGameOver && (
        <Pressable style={[StyleSheet.absoluteFillObject, styles.gameOverBg]}>
          <View style={styles.gameOverCard}>
            <LinearGradient
              colors={gameState.winner === 'p1' ? ['#1E293B', '#1A3320'] : ['#1E293B', '#331A1A']}
              style={styles.gameOverGradient}
            >
              <MaterialCommunityIcons
                name={gameState.winner === 'p1' ? 'trophy' : 'skull-crossbones'}
                size={40}
                color={gameState.winner === 'p1' ? '#F59E0B' : '#EF4444'}
              />
              <Text style={[styles.gameOverTitle, { color: gameState.winner === 'p1' ? '#22C55E' : '#EF4444' }]}>
                {gameState.winner === 'p1' ? 'VICTORY!' : 'DEFEAT'}
              </Text>
              <Text style={styles.gameOverSub}>
                {gameState.winner === 'p1' ? 'You defeated your opponent!' : 'Better luck next time!'}
              </Text>

              <View style={styles.gameOverStats}>
                <View style={styles.gameOverStatBox}>
                  <Text style={styles.gameOverStatLabel}>Your HP</Text>
                  <Text style={[styles.gameOverStatValue, { color: '#22C55E' }]}>{p.hp}</Text>
                </View>
                <View style={styles.gameOverStatBox}>
                  <Text style={styles.gameOverStatLabel}>Rounds</Text>
                  <Text style={styles.gameOverStatValue}>{gameState.roundResults.length}</Text>
                </View>
                <View style={styles.gameOverStatBox}>
                  <Text style={styles.gameOverStatLabel}>Opp HP</Text>
                  <Text style={[styles.gameOverStatValue, { color: '#EF4444' }]}>{opp.hp}</Text>
                </View>
              </View>

              <View style={styles.gameOverButtons}>
                <Pressable
                  style={[styles.gameOverBtnPrimary, rematchRequested && { opacity: 0.5 }]}
                  onPress={handleRematch}
                  disabled={rematchRequested}
                >
                  <Ionicons name="refresh" size={16} color="#fff" />
                  <Text style={styles.gameOverBtnText}>
      ...
      {/* === DIALOGS (Log / History) — inline overlay === */}
      {(showDialog === 'log' || showDialog === 'history') && (
        <Pressable style={[StyleSheet.absoluteFillObject, styles.dialogBg]} onPress={() => setShowDialog('none')}>
          <View style={[styles.dialogCard, { marginTop: safeT + 16, marginBottom: safeB + 16, marginLeft: safeL + 16, marginRight: safeR + 16 }]}>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12, paddingBottom: 20 }}>
              {showDialog === 'log' && gameState.log.map((l, i) => (
                    ...
                    <View key={i} style={styles.historyItem}>
                      ...
                      {r.abilityEffects && r.abilityEffects.length > 0 && (
                        <View style={styles.historyTraits}>
                          {r.abilityEffects.map((ae: any, aei: number) => {
          ...
          <View style={styles.cardPopup}>
            <View style={[styles.cardPopupInner, { borderColor: ELEMENT_SOLID_BORDERS[(popupCard.element || '').toLowerCase()] ?? '#7C3AED80' }]}>
              {popupCard.id ? (
                ...
                <LinearGradient
                  colors={[getElementBg(popupCard.element || 'fire'), '#0F172A']}
      ...
      {(showDialog === 'commanderInfo' || showDialog === 'oppCommanderInfo') && (
        <Pressable style={[StyleSheet.absoluteFillObject, styles.dialogBg]} onPress={() => setShowDialog('none')}>
          <View style={styles.cmdInfoOverlay}>
            {(() => {
              const cmd = showDialog === 'commanderInfo' ? p.commander : opp.commander;
      ...
      {opponentDisconnected && (
        <View style={[StyleSheet.absoluteFillObject, styles.disconnectOverlay]}>
          <View style={styles.disconnectCard}>
            <MaterialCommunityIcons name="wifi-off" size={32} color="#F59E0B" />
            <Text style={styles.disconnectTitle}>Opponent Disconnected</Text>
            <Text style={styles.disconnectTimer}>{disconnectTimer}s</Text>
            <Text style={styles.disconnectSub}>
              {disconnectTimer > 0 ? 'Waiting for opponent to reconnect...' : 'Opponent forfeited!'}
            </Text>
          </View>
        </View>
      )}
      ...
      {!ws.isConnected && (
        <View style={[StyleSheet.absoluteFillObject, styles.disconnectOverlay]}>
          <View style={styles.disconnectCard}>
            <ActivityIndicator size="large" color="#7C3AED" />
            <Text style={styles.disconnectTitle}>Reconnecting...</Text>
            <Text style={styles.disconnectSub}>Connection lost. Attempting to reconnect.</Text>
          </View>
        </View>
      )}
    </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  loadingText: { fontFamily: 'Inter_600SemiBold', fontSize: 14, color: '#94A3B8', marginTop: 12 },
  ...
  hpBarWrap: {
    ...
    borderWidth: 1, borderRadius: 5, paddingHorizontal: 4, paddingVertical: 2, overflow: 'hidden',
  },
  ...
  headerLogBtn: {
    ...
    paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4, backgroundColor: '#1E293B',
  },
  ...
  oppHandRow: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#EF444415', backgroundColor: '#EF444008', overflow: 'hidden',
  },
  ...
  fieldZone: {
    borderRadius: 6, borderWidth: 1, paddingHorizontal: 4, paddingVertical: 2, overflow: 'hidden', marginHorizontal: 2,
  },
  ...
  bfCard: { aspectRatio: 3 / 4, borderRadius: 6, borderWidth: 2, overflow: 'hidden' },
  ...
  combatResultRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, flex: 1 },
  combatResultBanner: {
  ...
  waitingText: { fontFamily: 'Inter_600SemiBold', fontSize: 10, color: '#7C3AED' },

  vsDivider: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, flex: 1 },
  vsLine: { flex: 1, height: 1, backgroundColor: '#EF444420' },
  vsBadge: {
    width: 18, height: 18, borderRadius: 9, backgroundColor: '#1E293B',
  ...
  bottomTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: 22 },
  ...
  battleButton: {
    ...
    shadowColor: '#EF4444', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.4, shadowRadius: 4, elevation: 4,
  },
  ...
  handCardNameText: { fontFamily: 'Inter_500Medium', color: '#CBD5E1', textAlign: 'center' },

  abilityCardH: {
    backgroundColor: '#1E293B', borderRadius: 6, padding: 5,
    borderWidth: 1, borderColor: '#7C3AED30', gap: 1, width: 140,
  },
  ...
  dialogCard: {
    ...
    borderWidth: 2, borderColor: '#7C3AED30', maxWidth: 460, maxHeight: '88%',
  },
  dialogHeader: {
  ...
  disconnectCard: {
    backgroundColor: '#1E293B', borderRadius: 16, padding: 24, alignItems: 'center', gap: 8,
    borderWidth: 2, borderColor: '#F59E0B40', maxWidth: 280,
  },
  disconnectTitle: { fontFamily: 'Inter_700Bold', fontSize: 16, color: '#F59E0B' },
  disconnectTimer: { fontFamily: 'Inter_700Bold', fontSize: 32, color: '#EF4444' },
  disconnectSub: { fontFamily: 'Inter_400Regular', fontSize: 11, color: '#94A3B8', textAlign: 'center' },
